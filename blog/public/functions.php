...
function tree($arr, $id = 'cid', $pid = 'pid')
{
    ...
    foreach ($items as $item) {
        ...
        //arr[0][son ][1]=arr[1];
        //加引用赋值是为了同步修改 $items,否则son 就不会赋值到$items 里
    }
...
function getPageList($sql = '' ,  $fields = array() , $mod = array() , $func = 'getTable' , $query = array())
{
    // 1.总条数 count(总数据)
    // 2.每页条数 ,建议定义常量配置
    // 3.页数 ceil(总条数 / 每页条数)
    ...
    $html = $func($datas , $fields , $mod); //获取表格
...
// select * from article where is_show = 1 && cid = $_GET['cid']
function getList($data = array())
{
    ...
    foreach($data as $d){
        ...
        $html .='<li>
						<p class="list-title">
							<a href="show.php?aid='.$d['aid'].'">'.getSubTitle($d['title'] , 30).'</a>
						</p>
						<p class="list-state">
							更新时间 : '.date('Y-m-d' , $d['update_time']).' 作者 : '.$d['author'].' 分类 : '.$d['cate_name'].' 浏览 : '.$d['clicks'].'
						</p>
						<p class="list-desc">
							'.getSubTitle($d['description'] , 120).'
						</p>
					</li>';
    }
    return $html;
...
function prevArticle($aid , $cid)
{
    $html = '没有了!';
    $data = DB('SELECT aid,title FROM article WHERE is_show = 1 && cid = '.$cid.' && aid < '.$aid.' ORDER BY aid DESC LIMIT 1');
    if(!empty($data))
    {
        $html = '<a href="show.php?aid='.$data[0]['aid'].'">'.$data[0]['title'].'</a>';
    }
    return $html;
}

function nextArticle($aid , $cid)
{
    $html = '没有了!';
    $data = DB('SELECT aid,title FROM article WHERE is_show = 1 && cid = '.$cid.' && aid > '.$aid.' ORDER BY aid ASC LIMIT 1');
    if(!empty($data))
    {
        $html = '<a href="show.php?aid='.$data[0]['aid'].'">'.$data[0]['title'].'</a>';
    }
